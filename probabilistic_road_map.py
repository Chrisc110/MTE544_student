...
def prm_graph(start, goal, obstacles_list, robot_radius, *, rng=None, m_utilities=None):
    ...
    if show_plot:
        if use_map:
            ...
            # plot the sample points
            plt.plot(sample_points[0], sample_points[1], ".b", label="Sample Point")   
            # Plot list of obstacles
            plt.plot(obstacles_list[:,0], obstacles_list[:,1], ".k", label="Obstacle")  
            # Plot the roadmap
...
def generate_sample_points(start, goal, rr, obstacles_list, obstacle_kd_tree, rng):
    ...
    min_x, max_x = ox.min(), ox.max()
    min_y, max_y = oy.min(), oy.max()
    ...
    while len(sample_x) < N_SAMPLE:
        ...
        # check if point is not too close to any obstacles
        distance, _ = obstacle_kd_tree.query([rx, ry])
        if distance > rr:
            sample_x.append(int(round(rx)))
            sample_y.append(int(round(ry)))

    
    # [Part 2] TODO Add also the start and goal to the samples so that they are connected to the roadmap
    sample_x.append(int(round(sx)))
    sample_y.append(int(round(sy)))
    sample_x.append(int(round(gx)))
    sample_y.append(int(round(gy)))
...
def is_collision(sx, sy, gx, gy, rr, obstacle_kd_tree, max_edge_len):
    ...
    # euclidean distance between start and goal node
    edge_length = np.hypot(gx - sx, gy - sy)

    # check edge length
    if edge_length > max_edge_len:
        return True

    # create points along line to sample
    num_samples = int(np.ceil(edge_length / rr))
    x_samples = np.linspace(sx, gx, num_samples)
    y_samples = np.linspace(sy, gy, num_samples)

    # check for collisions
    for x, y in zip(x_samples, y_samples):
        distance, _ = obstacle_kd_tree.query([x, y])
        if distance <= rr:
            return True
...
def generate_road_map(sample_points, rr, obstacle_kd_tree, max_edge_len, m_utilities=None):
    ...
    # Create a KDTree for the sample points to find nearest neighbors
    sample_kd_tree = KDTree(np.vstack((sample_x, sample_y)).T)

    for i in range(n_sample):
        #find neighbors
        neighbors = sample_kd_tree.query(np.array([sample_x[i], sample_y[i]]), k=N_KNN + 1)[1] # add 1 to include itself
        edges = []

        for j in neighbors:
            if i == j:
                continue  # skip if yourself

            sx, sy = sample_x[i], sample_y[i]
            gx, gy = sample_x[j], sample_y[j]

            # check for collision and max length
