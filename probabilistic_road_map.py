...
def generate_sample_points(start, goal, rr, obstacles_list, obstacle_kd_tree, rng):
    ...
    while len(sample_x) < N_SAMPLE:
        ...
        # Check if the point is in free space (not too close to obstacles)
...
def is_collision(sx, sy, gx, gy, rr, obstacle_kd_tree, max_edge_len):
    ...
    # Calculate the Euclidean distance between the start and goal points
    edge_length = np.hypot(gx - sx, gy - sy)

    # Check if the edge length exceeds the maximum allowed length
    if edge_length > max_edge_len:
        return True

    # Number of intermediate points to sample along the edge
    ...
    # Check for collisions at each sampled point
...
def generate_road_map(sample_points, rr, obstacle_kd_tree, max_edge_len, m_utilities=None):
    ...
    for i in range(n_sample):
        neighbors = sample_kd_tree.query(np.array([sample_x[i], sample_y[i]]), k=N_KNN + 1)[1]  # +1 to include itself
        edges = []
        ...
        for j in neighbors:
            ...
            # Check for collision and max edge length
